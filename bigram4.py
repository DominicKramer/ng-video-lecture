...
class Config(object):
    def __init__(self,
                 ...
                 device: str,
                 eval_iters: int,
                 loss_estimate_percentage: float):
        ...
        self.loss_estimate_percentage = loss_estimate_percentage
...
def get_train_dataloader(text: str, tokenizer: Tokenizer, config: Config):
    train_dataset, _ = get_train_val_datasets(text, tokenizer, config)
    return DataLoader(
...
def get_random_subset(dataset: TensorDataset, config: Config) -> Dataset:
    size = 10000 # int(len(dataset)*config.batch_size)
    indices = [random.randint(0, len(dataset) + 1) for _ in range(size)]
    return Subset(dataset, indices)


def get_random_subset_train_val_dataloaders(text: str,
                                            tokenizer: Tokenizer,
                                            config: Config):
    train_dataset, val_dataset = get_train_val_datasets(text, tokenizer, config)
    train_loader = DataLoader(
        dataset=get_random_subset(train_dataset, config),
        batch_size=config.batch_size,
        shuffle=True
    )
    val_loader = DataLoader(
        dataset=get_random_subset(val_dataset, config),
        batch_size=config.batch_size,
        shuffle=True
    )
...
def estimate_loss(model: nn.Module,
                  ...
                  tokenizer: Tokenizer,
                  config: Config):
    train_loader, val_loader = get_random_subset_train_val_dataloaders(text, tokenizer, config)
...
def train(model: nn.Module,
    ...
    train_loader = get_train_dataloader(text, tokenizer, config)
